...
const FIELD_LEGEND_VARIANT = {
  LEGEND: "legend",
  LABEL: "label",
} as const

type FieldLegendVariant =
  ValueOf<typeof FIELD_LEGEND_VARIANT>
...
function FieldLegend({
  className,
  variant = FIELD_LEGEND_VARIANT.LEGEND,
  ...props
  ...
  return (
    <legend
      ...
      className={cn(
        "text-text-body mb-3 font-medium data-[variant=label]:text-sm data-[variant=legend]:text-base",
        className
      )}
      {...props}
    />
...
function FieldTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="field-label"
      className={cn(
        "text-text-body gap-2 text-sm font-medium group-data-[disabled=true]/field:opacity-50 flex w-fit items-center leading-snug",
        className
      )}
      {...props}
    />
