        ...
        <el-form 
          ...
          <el-form-item>
            <el-button-group>
              ...
              <el-button type="success" @click="setDefaultPicBed('gitee')" round :disabled="defaultPicBed === 'gitee'">设为默认图床</el-button>
            </el-button-group>
          </el-form-item>
        </el-form>
      </el-col>
...
export default {
  ...
  created () {
    const config = this.$db.get('picBed.gitee').value()
    if (config) {
      for (let i in config) {
  ...
  methods: {
    confirm () {
      this.$refs.gitee.validate((valid) => {
        if (valid) {
          this.$db.set('picBed.gitee', this.form).write()
